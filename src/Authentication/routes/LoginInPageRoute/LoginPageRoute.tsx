...
class LoginPageRoute extends React.Component<AuthenticationProps> {
   ...
   onClickLogin = async (event: { preventDefault: () => void }) => {
      if (this.props.authenticationStore.userName === '') {
         ...
         this.props.authenticationStore.userNameErrorMessage =
            strings.errorMessageUserName
      ...
      if (this.props.authenticationStore.password === '') {
         event.preventDefault()
         this.props.authenticationStore.passwordErrorMessage =
            strings.errorMessagePAssword
         if (this.props.authenticationStore.userName === '') {
            if (this.userNameRef.current) {
               this.userNameRef.current.focus()
            }
   ...
   render() {
      ...
      if (accessToken) {
         return <Redirect to={COVID19_DASHBOARD} />
      }
      return (
         <LoginPage
